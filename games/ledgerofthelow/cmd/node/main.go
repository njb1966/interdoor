// Command node runs an InterDOOR node: an SSH server hosting a terminal game.
package main
...
func main() {
	...
	log.Printf("InterDOOR node %q on %s — game %q (max %d sessions, idle %ds)",
	...
	if cfg.HubURL != "" {
		...
		if apiKey == "" {
			...
			apiKey, err = client.Register(cfg.HubRegToken, cfg.Node, g.ID(), game.Version, cfg.AdvertiseAddr, g.Title())
			if err != nil {
		...
		syncer := &fed.Syncer{Store: store, Client: client, NodeID: cfg.Node, GameVersion: game.Version}
		syncer.PvPResolve = g.IncomingPvP
		syncer.TravelImport = func(st *engine.Store, snap *engine.Snapshot, fromNode, destNode string) error {
			...
			return st.Emit("player.traveled", map[string]any{
				...
				"src_node":      fromNode,
				"dest_node":     destNode,
				"snapshot_hash": fmt.Sprintf("%x", sha256.Sum256(snapJSONForHash(snap))),
	...
	// Graceful shutdown: stop accepting, then let the deferred store.Close run.
...
func snapJSONForHash(snap *engine.Snapshot) []byte {
